        ...
        $.post('/ajax/vote/get_voters', { id: cid, type: type, filter: filter }, function (data) {
            if (data.status == 'ok') {
                ...
                data.voters.forEach(function (vote) {
                    ...
                    if (vote.up) content += '<span class="fa fa-arrow-up vote-up"></span>';else content += '<span class="fa fa-arrow-down vote-down"></span>';
...
$(document).ready(function () {
    ...
    if (AppData.user && window.WebSocket) {
        ...
        var subscribeToEntryReply = function subscribeToEntryReply($this) {
            pusher.subscribe('entry.' + $this.data('id')).bind('new-reply', function (data) {
                if (window.blocked_users.indexOf(data.author) != -1) return;
